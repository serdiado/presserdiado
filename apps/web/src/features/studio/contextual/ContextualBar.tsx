...
// ─── Footer Konteyneri Seçildiğinde Hızlı Erişim Barı ────────────────────────
function FooterContainerMode() {
  const selection = useUIStore((s) => s.selection);
  const getActivePages = useCatalogStore((s) => s.getActivePages);
  const globalSettings = useCatalogStore((s) => s.globalSettings);
  ...
  // Clipboard — oturum boyunca yaşar, FooterCellMode ile paylaşılır
  ...
  const scope: number | 'global' = isCustom && !isNaN(pageNum) ? pageNum : 'global';

  const btnCls =
    'h-9 px-3 inline-flex items-center gap-1.5 rounded-md text-xs font-medium whitespace-nowrap hover:bg-border-default transition-colors disabled:opacity-30';

  const handleEditClick = () => {
    // FooterRenderer'daki "Alt bilgiyi düzenle" overlay butonuyla aynı işi yapar.
    // UI'da footer'ın ilk görünür hücresini seçili yaparak edit moduna girmesini tetikler.
    // Gerçek edit-mode tetiklemesi FooterRenderer içinde kalıyor; burada sadece
    // sidebar'ı doğru sekmeye götürüyoruz.
    setSidebarState('grid', 'cell');
  };

  return (
    <>
      {/* Kapsam — sadece sayfa seçiliyken (pageNum geçerliyse) göster */}
      {!isNaN(pageNum) && (
        <>
          <div className={`inline-flex items-center gap-2 px-2 py-1 rounded-md transition-colors ${isCustom ? 'bg-blue-50' : 'bg-surface-subtle'}`}>
            <span className={`text-xs font-medium whitespace-nowrap transition-colors ${!isCustom ? 'text-text-primary' : 'text-text-muted'}`}>
            ...
            <label className="relative inline-flex items-center cursor-pointer">
              <input
                ...
                className="sr-only peer"
                checked={isCustom}
                onChange={() => setPageFooterMode(pageNum, isCustom ? 'global' : 'custom')}
              />
              <div className="w-9 h-5 bg-border-strong rounded-full peer peer-checked:bg-blue-500 after:content-[''] after:absolute after:top-0.5 after:left-0.5 after:bg-white after:rounded-full after:h-4 after:w-4 after:transition-all peer-checked:after:translate-x-4" />
            </label>
            <span className={`text-xs font-medium whitespace-nowrap transition-colors ${isCustom ? 'text-blue-700' : 'text-text-muted'}`}>
      ...
      {/* Yükseklik */}
      <Popover trigger={<><RulerIcon size={16} /><span>Yükseklik</span></>} width="w-52">
        <div className="flex flex-col gap-2">
          <span className="text-[10px] font-bold text-text-secondary">Yükseklik (mm)</span>
          <div className="flex items-center gap-2">
            <input
              type="range"
              min={5}
              ...
              value={heightMm}
              onChange={(e) =>
                updateFooterSettings(scope, { heightMm: parseInt(e.target.value) })
              }
              className="flex-1 studio-slider"
            />
            <input
              ...
              value={heightMm}
              onChange={(e) =>
                updateFooterSettings(scope, { heightMm: parseInt(e.target.value) || 5 })
              }
              className="w-12 text-xs font-normal text-text-primary text-center border border-border-default rounded p-0.5"
            />
      ...
      <ColorOpacityPicker
        trigger={
          <>
            <div
              className="w-3.5 h-3.5 rounded-sm shrink-0"
              style={{
                ...colorValueBackground(activeFooter?.bgColor ?? { type: 'solid', color: '#ffffff', opacity: 0 }),
                border: '1px solid rgba(0,0,0,0.15)',
                borderRadius: '4px',
              }}
            />
        ...
        value={activeFooter?.bgColor ?? { type: 'solid', color: '#ffffff', opacity: 0 }}
        onChange={(v) => updateFooterSettings(scope, { bgColor: v })}
      />
        ...
        trigger={
          <>
            <div
              ...
              style={{
                backgroundColor: 'transparent',
                border: `2px solid ${colorOpacityToCss(activeFooter?.containerBorder?.color ?? { c: '#e2e8f0', o: 100 })}`,
                borderRadius: '4px',
              }}
            />
            <span>Çerçeve</span>
      ...
      >
        <div className="flex flex-col gap-3">
          <div className="flex items-center justify-between">
            ...
            <ColorOpacityPicker
              solidOnly
              value={{ type: 'solid', color: activeFooter?.containerBorder?.color?.c ?? '#e2e8f0', opacity: activeFooter?.containerBorder?.color?.o ?? 100 }}
              onChange={(v) => {
                if (v.type !== 'solid') return;
                const cb = activeFooter?.containerBorder ?? { color: { c: '#e2e8f0', o: 100 }, width: 0 };
                updateFooterSettings(scope, {
          ...
          <div className="flex items-center justify-between gap-3 pt-1 border-t border-border-default">
            ...
            <div className="flex items-center gap-2">
              <input
                type="range" min={0} max={10} step={1} value={activeFooter?.containerBorder?.width ?? 0}
                onChange={(e) => {
                  const cb = activeFooter?.containerBorder ?? { color: { c: '#e2e8f0', o: 100 }, width: 0 };
                  updateFooterSettings(scope, {
                    containerBorder: { ...cb, width: parseInt(e.target.value, 10) || 0 },
                  });
                }}
                className="w-24 studio-slider"
              />
              <span className="text-xs font-medium w-8 text-right">{activeFooter?.containerBorder?.width ?? 0}mm</span>
            </div>
          </div>
        </div>
      </Popover>
      ...
      <Popover trigger={<><CornerRadiusIcon size={16} />Köşe</>} width="w-72">
        <BorderRadiusPicker
          value={activeFooter?.radius ?? { tl: 0, tr: 0, bl: 0, br: 0, linked: true }}
          onChange={(v) => updateFooterSettings(scope, { radius: v })}
        />
      </Popover>
      ...
      <button
        className={btnCls}
        onClick={() => setSidebarState('grid', 'cell')}
      >
        <Settings size={16} />
        Ayarlar
      </button>
...
// Toolbar clipboard — FooterContainerMode and FooterCellMode arasında paylaşılır.
// Bileşen dışında tanımla, oturum boyunca yaşaması için.
let _footerToolbarClipboard: { heightMm: number; cells: any[] } | null = null;

// ─── Footer Hücresi Seçildiğinde Hızlı Erişim Barı ─────────────────────────────
function FooterCellMode() {
  const selection = useUIStore((s) => s.selection);
  ...
  const scope: number | 'global' = isCustom ? pageNum : 'global';
  const activeFooter = isCustom ? page?.customFooter : globalSettings.footer;
  const cells = activeFooter?.cells ?? [];

  if (selectedCellIds.length === 0) return null;

  const targetCells = cells.filter((c: any) => selectedCellIds.includes(c.id));
  ...
  const updateCells = (patch: Partial<any>) => {
    selectedCellIds.forEach((id) => {
      updateFooterCellStore(scope, id, patch);
    });
  };
  ...
  const handleFileSelect = (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (event) => {
      ...
      selectedCellIds.forEach((id) => {
        updateFooterCellStore(scope, id, { image: base64 });
      });
    };
  ...
  const mergeCells = () => {
    mergeFooterCellsStore(scope, selectedCellIds);
  };

  const splitCell = () => {
    if (selectedCellIds[0]) {
      unmergeFooterCellStore(scope, selectedCellIds[0]);
    }
  };
  ...
  const resetCell = () => {
    selectedCellIds.forEach((id) => {
      updateFooterCellStore(scope, id, {
        ...
        padding: { t: 2, r: 2, b: 2, l: 2, linked: true },
        image: null,
        text: '',
      });
    });
  ...
  return (
    <>
      ...
      ) : selectedCellIds.length === 1 && isMerged ? (
        <button
          onClick={splitCell}
          className={btnCls}
        >
      ...
      {firstCell && (
        <>
          ...
          <ColorOpacityPicker
            ...
            trigger={
              <>
                <div
                  className="w-3.5 h-3.5 rounded-sm shrink-0"
                  style={{
                    backgroundColor: firstCell.bgColor ? colorOpacityToCss(firstCell.bgColor) : 'transparent',
                    border: '1px solid rgba(0,0,0,0.15)',
                    borderRadius: '4px',
                  }}
                />
            ...
            value={firstCell.bgColor ? { type: 'solid', color: firstCell.bgColor.c, opacity: firstCell.bgColor.o } : { type: 'solid', color: '#ffffff', opacity: 100 }}
            onChange={(v) => {
              if (v.type !== 'solid') return;
              updateCells({ bgColor: { c: v.color, o: v.opacity } });
            }}
          />
            ...
            trigger={
              <>
                <div
                  ...
                  style={{
                    backgroundColor: 'transparent',
                    border: `2px solid ${colorOpacityToCss(firstCell.border?.color ?? { c: '#e2e8f0', o: 100 })}`,
                    borderRadius: '4px',
                  }}
                />
                <span>Çerçeve</span>
          ...
          >
            <div className="flex flex-col gap-3">
              <div className="flex items-center justify-between">
                ...
                <ColorOpacityPicker
                  solidOnly
                  value={{ type: 'solid', color: firstCell.border?.color?.c ?? '#e2e8f0', opacity: firstCell.border?.color?.o ?? 100 }}
                  onChange={(v) => {
                    if (v.type !== 'solid') return;
              ...
              <div className="flex items-center justify-between gap-3 pt-1 border-t border-border-default">
                ...
                <div className="flex items-center gap-2">
                  <input
                    type="range" min={0} max={10} step={1} value={firstCell.border?.t ?? 0}
                    onChange={(e) => {
                    ...
                    className="w-24 studio-slider"
                  />
                  <span className="text-xs font-medium w-8 text-right">{firstCell.border?.t ?? 0}px</span>
                </div>
              </div>
            </div>
          </Popover>
          ...
          <input
            type="number" min={6} max={120} value={firstCell.font?.fontSize ?? 12}
            onChange={(e) => updateCells({ font: { ...firstCell.font, fontSize: parseInt(e.target.value) || 12 } })}
            className="w-14 text-center text-xs border border-border-default rounded-md px-1 py-1.5"
          />

          <select
            value={firstCell.font?.fontWeight ?? '400'}
          ...
          <ColorOpacityPicker
            solidOnly
            trigger={<ColorSwatchTrigger color={firstCell.font?.color ?? '#000000'} opacity={firstCell.font?.opacity ?? 100} />}
            value={{ type: 'solid', color: firstCell.font?.color ?? '#000000', opacity: firstCell.font?.opacity ?? 100 }}
            onChange={(v) => {
              if (v.type !== 'solid') return;
          ...
          <BannerTextAlignDropdown
            textAlign={(firstCell.font?.textAlign as any) || 'center'}
